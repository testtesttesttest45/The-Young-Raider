...
class Catastrophe {
  ...
  constructor(scene: any, baseLevel: number) {
    ...
    this.damage = Math.round(
      this.baseDamage * Math.pow(1.2, this.baseLevel - 1),
    );
  ...
  launchStorm() {
    if (!this.isStorming) {
      ...
      this.activeStormEffects = 0;
      this.fireballPositions = [];
      audioManager.playSound("sfx-catastrophe", 0.75);
      for (let i = 0; i < 6; i++) {
        ...
        let indicator = this.scene.add.circle(
          x,
          endY,
          indicatorRadius,
          0xff0000,
        ...
        this.scene.tweens.add({
          ...
          onComplete: () => {
            let fireball = this.scene.add
              .sprite(x, startY, "fireball")
              .setScale(0.5);
            fireball.setDepth(2);
            ...
            this.scene.tweens.add({
              ...
              onComplete: () => {
                ...
                this.indicators = this.indicators.filter(
                  (item: any) => item.indicator !== indicator,
                );
                this.activeStormEffects--;
  ...
  checkDamage(x: number, y: number, radius: number) {
    ...
    this.scene.enemies.forEach((enemy: any) => {
      ...
      let distanceToEnemy = Phaser.Math.Distance.Between(
        x,
        y,
        enemyPos.x,
        enemyPos.y,
  ...
  update(time: number, delta: number) {
    if (this.scene.activeGameTime > this.fireballTimer && !this.isStorming) {
      this.launchStorm();
    }
  }
}
