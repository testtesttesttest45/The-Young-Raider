...
export default class BattleShop {
  ...
  reset(): void {
    ...
    this.upgradeTooltip = null;
    this.mask = null;
    if (Array.isArray(this.legendaryIcons)) {
      this.legendaryIcons.forEach(({ icon }) => {
        if (icon?.active) {
          icon.destroy();
        }
      });
  ...
  update(): void {
    ...
    this.buyButtons.forEach((item: any) => {
      ...
      if (
        this.purchaseCounts.hasOwnProperty(item.upgradeName) &&
        this.purchaseCounts[item.upgradeName]! >= this.itemPurchaseLimit
      ) {
      ...
      if (
        this.legendaryPurchaseCount.hasOwnProperty(item.upgradeName) &&
        this.legendaryPurchaseCount[item.upgradeName]! >=
  ...
  createShopModal(): void {
    ...
    this.shopModalContainer = this.ui.add
      .container(0, 0)
      .setDepth(1000)
      .setVisible(false);

    this.invisibleBackground = this.ui.add
    ...
    this.headerBackground = this.ui.add
      .graphics()
      .fillStyle(0x1d3447, 1)
      .fillRoundedRect(modalX, modalY, modalWidth, headerHeight, {
        tl: 16,
        tr: 16,
        bl: 0,
        br: 0,
    ...
    this.scrollableContainer = this.ui.add
      .container(0, 0)
      .setDepth(1001)
      .setVisible(false);

    this.shopContentCamera = this.ui.cameras.add(
      viewportX,
      viewportY,
      viewportWidth,
      viewportHeight,
    ...
    this.shopContentCamera
      .setScroll(viewportX, viewportY)
      .setBackgroundColor("rgba(0,0,0,0)")
      .setVisible(false);

    this.shopHeaderContainer = this.ui.add
      .container(0, 0)
      .setDepth(1002)
      .setVisible(false);
    ...
    this.attackSpeedSectionTitle = this.createShopSectionTitle(
      leftColumnX,
      leftY,
      "ATTACK SPEED",
    );
    ...
    this.healthSectionTitle = this.createShopSectionTitle(
      rightColumnX,
      rightY,
      "HEALTH",
    );
    ...
    this.movementSpeedSectionTitle = this.createShopSectionTitle(
      rightColumnX,
      rightY,
      "MOVEMENT SPEED",
    );
    ...
    this.legendaryUpgradesSectionTitle = this.createShopSectionTitle(
      viewportX,
      legendaryY,
      "LEGENDARY UPGRADES",
    );
    ...
    this.scrollbarTrack = this.ui.add
      .rectangle(trackX, trackY, scrollbarWidth, viewportHeight, 0xffffff, 0.16)
      .setOrigin(0.5, 0);
    ...
    this.scrollbarHandle = this.ui.add
      .rectangle(trackX, trackY, scrollbarWidth, handleHeight, 0x50c8ff, 0.9)
      .setOrigin(0.5, 0)
      .setInteractive({
        useHandCursor: true,
      });
    ...
    const isInsideShopModal = (pointer: Phaser.Input.Pointer): boolean => {
      return (
        pointer.x >= modalX &&
        pointer.x <= modalX + modalWidth &&
        pointer.y >= viewportY &&
        pointer.y <= modalY + modalHeight
      );
    };
    ...
    const updateScrollbarFromContent = (): void => {
      ...
      this.scrollbarHandle.y =
        trackY + Phaser.Math.Clamp(ratio, 0, 1) * handleRange;
    };
    ...
    const updateContentFromScrollbar = (): void => {
      ...
      this.scrollableContainer.y =
        Phaser.Math.Clamp(ratio, 0, 1) * minimumScrollY;
    };
    ...
    this.ui.input.on(
      ...
      (
        ...
        if (
          ...
          currentlyOver.includes(this.scrollbarHandle)
        ) {
          return;
        }
    ...
    this.ui.input.on("pointermove", (pointer: Phaser.Input.Pointer) => {
      if (
        !this.shopModalContainer?.visible ||
        activePointerId !== pointer.id ||
        isDraggingHandle ||
        !pointer.isDown ||
    ...
    this.ui.input.on(
      ...
      (
        ...
        if (
          !this.shopModalContainer?.visible ||
          !this.scrollableContainer ||
          !isInsideShopModal(pointer)
        ) {
          return;
        }

        this.scrollableContainer.y = Phaser.Math.Clamp(
          this.scrollableContainer.y - deltaY * 0.65,
          minimumScrollY,
          0,
        );
    ...
    const objectsIgnoredByShopCamera = this.ui.children.list.filter(
      (gameObject) => gameObject !== this.scrollableContainer,
    );
  ...
  createItems(
    ...
    upgrades.forEach((upgrade, index) => {
      ...
      const descriptionText = this.ui.add.text(
        startX + 58,
        itemY + 35,
        upgrade.description,
        {
      ...
      if (this.purchaseCounts.hasOwnProperty(upgrade.name)) {
        const purchaseCountText = this.ui.add
          .text(
            startX + itemWidth - 10,
            itemY + 8,
            `(${this.purchaseCounts[upgrade.name] ?? 0}/${this.itemPurchaseLimit})`,
            {
              font: "10px Orbitron",
              color: "#ffd84a",
            },
        ...
        const purchaseCountText = this.ui.add
          .text(
            startX + itemWidth - 10,
            itemY + 8,
            `(${this.legendaryPurchaseCount[upgrade.name] ?? 0}/${this.legendaryPurchaseLimit})`,
            {
              font: "10px Orbitron",
              color: "#ffd84a",
            },
  ...
  bulkPurchasePenknife(cost: number) {
    if ((this.ui.scene.get("Game") as any).player.currentHealth <= 0) {
      this.showPurchaseFeedback(
        "You are dead! You cannot purchase upgrades",
        "#ff0000",
      );
      return;
    }
    ...
    if (maxPurchases > 0) {
      ...
      this.showPurchaseFeedback(
        `Bought ${maxPurchases} Penknives! +${maxPurchases} Damage`,
        "#00ff00",
      );
    }
  }
    ...
    if (this.ui.gold >= cost) {
      if (this.purchaseCounts.hasOwnProperty(upgradeName)) {
        if (this.purchaseCounts[upgradeName]! < this.itemPurchaseLimit) {
          ...
          const item = this.buyButtons.find(
            (item) => item.upgradeName === upgradeName,
          );
          if (item && item.purchaseCountText) {
            item.purchaseCountText.setText(
              `(${this.purchaseCounts[upgradeName]}/${this.itemPurchaseLimit})`,
            );
          }
        } else {
          this.showPurchaseFeedback(
            `Limit reached for ${upgradeName}`,
            "#ff0000",
          );
          return;
        }
      ...
      if (
        ...
        if (
          ...
          if (item && item.purchaseCountText) {
            item.purchaseCountText.setText(
              `(${this.legendaryPurchaseCount[upgradeName]}/${this.legendaryPurchaseLimit})`,
            );
          }
        } else {
          this.showPurchaseFeedback(
      ...
      if (upgradeName === "Hunter's Blade") {
        (this.ui.scene.get("Game") as any).player.damage = Math.round(
          (this.ui.scene.get("Game") as any).player.damage * 1.02,
        );
      }
      if (upgradeName === "Heaven's Rain") {
        (this.ui.scene.get("Game") as any).player.maxHealth = Math.round(
          (this.ui.scene.get("Game") as any).player.maxHealth * 1.12,
        );
      }
      if (upgradeName === "Health Potion") {
        const healPercentage = 0.5;
        let healAmount = Math.round(
          (this.ui.scene.get("Game") as any).player.maxHealth * healPercentage,
        );
        healAmount = Math.min(
          healAmount,
          (this.ui.scene.get("Game") as any).player.maxHealth -
            (this.ui.scene.get("Game") as any).player.currentHealth,
        );
      ...
      if (upgradeName === "Lightning Core") {
        (this.ui.scene.get("Game") as any).player.speed = Math.round(
      ...
      if (
        ...
        if (!this.legendaryIcons.some((entry) => entry.name === upgradeName)) {
          ...
          if (!container) {
            console.error("[BattleShop] Legendary icon container is missing.");

            return;
          }

          const iconIndex = this.legendaryIcons.length;

          const iconsPerRow = 4;

          const column = iconIndex % iconsPerRow;

          const row = Math.floor(iconIndex / iconsPerRow);

          const iconSpacingX = 42;
          const iconSpacingY = 40;

          const localX = column * iconSpacingX;

          const localY = row * iconSpacingY;
          ...
          icon
            .on("pointerover", () => {
              const worldPosition = container.getWorldTransformMatrix();

              const tooltipX = worldPosition.tx + localX;

              const tooltipY = worldPosition.ty + localY - 22;

              this.showUpgradeTooltip(upgradeName, tooltipX, tooltipY);
            })
            .on("pointerout", () => {
              this.hideUpgradeTooltip();
            });

          this.legendaryIcons.push({
            name: upgradeName,
            icon,
          });
        }
      ...
      this.showPurchaseFeedback(
        `You need ${cost - this.ui.gold} more gold`,
        "#ff0000",
      );
    }
  }

  showUpgradeTooltip(upgradeName: string, x: number, y: number): void {
    this.hideUpgradeTooltip();
    ...
    if (this.shopContentCamera) {
      this.shopContentCamera.ignore(this.upgradeTooltip);
    }
  }
  ...
  showPurchaseFeedback(message: string, color: string = "#ffffff"): void {
    ...
    const feedbackText = this.ui.add
    ...
    this.currentFeedbackText = feedbackText;

    if (this.shopContentCamera) {
      this.shopContentCamera.ignore(feedbackText);
    }
    ...
    this.ui.tweens.add({
      ...
      onComplete: () => {
        ...
        if (this.currentFeedbackText === feedbackText) {
          this.currentFeedbackText = null;
        }
      },
    });
