...
export default class BattleShop {
    ...
    attackSpeedSectionTitle:
        Phaser.GameObjects.Text | null;

    movementSpeedSectionTitle:
        Phaser.GameObjects.Text | null;

    legendaryUpgradesSectionTitle:
        Phaser.GameObjects.Text | null;
    ...
    update(): void {
        if (this.penknifeBulkBuyButton) {
            const canAffordBulkBuy =
                this.ui.gold >= 55;

            this.penknifeBulkBuyButton.setText(
                canAffordBulkBuy
                    ? 'BULK BUY'
                    : ''
            );
    ...
    createShopModal(): void {
        ...
        this.ui.cameras.main.ignore(
            this.scrollableContainer
        );

        const objectsIgnoredByShopCamera =
            this.ui.children.list.filter(
                gameObject =>
                    gameObject !==
                    this.scrollableContainer
            );
    ...
    createItems(
        ...
        upgrades.forEach(
            (
                ...
                const descriptionText = this.ui.add
                    .text(
                        startX + 58,
                        itemY + 35,
                        upgrade.description,
    ...
    showUpgradeTooltip(
        ...
        this.upgradeTooltip = this.ui.add.text(x, y, upgradeName, {
            font: '18px Orbitron',
            color: '#ffffff',
    ...
    showPurchaseFeedback(
        ...
        const modalHeight = 560;
        const modalY =
            (this.ui.scale.height - modalHeight) / 2;

        const feedbackX =
            this.ui.scale.width / 2;
